...
150.times do |l|
    p = Produto.create()
    g = Geral.create(preco: (rand 5000) * 110, classificacao: (rand 7) + 3, nome: "produto-#{l} - #{(rand (1..85))}",descricao: "descricao extensiva nº -#{l} - #{(rand (1..85))}",serviceable_id: p.id, serviceable_type: p.class, loja_id: (rand (1..85)) + 1)
    p.save
    g.save
    p = Aluguer.create()
    g = Geral.create(preco: (rand 5000) * 110, classificacao: (rand 7) + 3, nome: "produto-#{l} - #{(rand (1..85))}",descricao: "descricao extensiva nº -#{l} - #{(rand (1..85))}",serviceable_id: p.id, serviceable_type: p.class, loja_id: (rand (1..85)) + 1)
